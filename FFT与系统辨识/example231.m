...
for k=1:L 
    ...
    y(k)=Phi'* Truetheta+xi(k); 	% Output data   每一次测量都有误差的混入  可以考虑对方差加权，作信息融入
...
for k = 1:L
    Ptr(k) = trace(Precoder{k});
end
figure,plot(Ptr)

% K 最终收敛到0  是因为 如果系统不存在误差，那么新的观测值的加入将不会影响原来的结果。
% K 一开始很大，表明新加入的观测占比很大
KList = [];
for k = 1:L
    KList(:,k) = (Krecoder{k});
end
figure,plot(KList(1,:),KList(2,:))
figure,plot(KList(3,:),KList(4,:))